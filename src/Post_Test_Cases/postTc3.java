...
public class postTc3 {

	// Method1=TestCaseExecutor
	@Test
	public static void post_Executer_Tc3() throws IOException {
		for (int i = 0; i < 3; i++) {
			int statuscode = postTc3.testResponseStatusCode();
			if (statuscode == 201) {
				String responseBody = postTc3.testResponseBody();
				postTc3.testResponseBodyValidator(responseBody);
				EvidenceFileCreator.evidencetxtfileCreator("postTc3_Evidence", postRequestRepository.Req_postTc3(),
						responseBody);
				break;
	...
	public static int testResponseStatusCode() throws IOException {
		...
		int responseStausCode = given().header("Content-Type", "application/json")
				.body(postRequestRepository.Req_postTc3()).when().post("/api/users").then().extract().statusCode();
	...
	public static String testResponseBody() throws IOException {
		...
		String responseBody = given().header("Content-Type", "application/json")
				.body(postRequestRepository.Req_postTc3()).when().post("/api/users").then().extract().response()
				.asString();
